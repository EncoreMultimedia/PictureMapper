...
export default class SystemTable extends Component {
    ...
    const tableBody = this.props.tableValue.map((rowData, index) => {
      ...
      return (
        rowData.header ? null : <tr key={rowData.id + 'imageSizes'}>
                                  ...
                                  <td><select onChange={(e)=>this.onChangeList(index, e)} defaultValue={rowData.breakpoint ? rowData.breakpoint : ''}>{list}</select></td>
  ...
  onChangeList(index, e) {
    this.props.callbacks.breakpointChange(index, e.target.value);
  }
